...
export class MultiplayerUI {
  ...
  _bindEvents() {
    ...
    this.elements.btnHost?.addEventListener('click', () => this._showCharSelect('host'));
    this.elements.btnJoin?.addEventListener('click', () => this._showCharSelect('join'));
    this.elements.btnMpBack?.addEventListener('click', () => this.hide());
    
    // Character selection
    this._setupGenderSelect();
    this.elements.btnCharConfirm?.addEventListener('click', () => this._confirmCharacter());
    this.elements.btnCharBack?.addEventListener('click', () => this._showModeSelect());
    
    // Join input
    this.elements.btnJoinConfirm?.addEventListener('click', () => this._joinRoom());
    this.elements.btnJoinBack?.addEventListener('click', () => this._showModeSelect());
    ...
    if (this.elements.btnStartGame) {
      this.elements.btnStartGame.addEventListener('click', () => this._startGame());
    ...
    if (this.elements.btnReady) {
      this.elements.btnReady.addEventListener('click', () => this._toggleReady());
      this.elements.btnReady.addEventListener('touchstart', (e) => {
        e.preventDefault();
        this._toggleReady();
      });
    }
    this.elements.btnLeaveRoom?.addEventListener('click', () => this._leaveRoom());
  ...
  _setupGenderSelect() {
    const genderBtns = this.elements.mpGenderSelect?.querySelectorAll('.toggle-btn');
    console.log('MultiplayerUI: Setting up gender select, buttons found:', genderBtns?.length);
    if (genderBtns) {
      genderBtns.forEach(btn => {
        ...
        btn.addEventListener('click', () => {
          ...
          console.log('MultiplayerUI: Gender selected:', this.playerProfile.gender);
        });
      });
    }
  }
  ...
  _confirmCharacter() {
    // Get player name
    const name = this.elements.mpPlayerName?.value.trim() || 'Player' + Math.floor(Math.random() * 10000);
  ...
  _showRoomLobby() {
    ...
    this.elements.lobbyTitle.textContent = this.mp.isHost ? 'YOUR ROOM' : 'ROOM LOBBY';
  ...
  _updatePlayerList() {
    ...
    localItem.innerHTML = `
      ...
      <span class="player-status ${isReady ? 'ready' : ''}">${isReady ? 'READY' : '●'}</span>
    `;
