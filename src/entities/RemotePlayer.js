...
export class RemotePlayer {
  constructor(
    ...
    // Initialize position from network data if provided
    const initPos = initialPosition || { x: 0, y: 0, z: 0 };

    // Target position (received from network)
    this.targetPosition = new THREE.Vector3(initPos.x, initPos.y, initPos.z);
    this.targetRotation = 0;

    // Current interpolated position - start at target to avoid teleporting
    this.currentPosition = new THREE.Vector3(initPos.x, initPos.y, initPos.z);
    ...
    // IMPORTANT: Set initial mesh position immediately
    this.mesh.position.copy(this.currentPosition);
    console.log(
      `[RemotePlayer] Created ${playerName} at (${initPos.x?.toFixed(
        1
      )}, ${initPos.y?.toFixed(1)}, ${initPos.z?.toFixed(1)})`
    );
  }
  ...
  _createMesh() {
    ...
    const colors =
      this.characterColors[this.playerGender] || this.characterColors.male;
    ...
    // Offset so feet are at Y=0 (center capsule so bottom touches ground)
    this.fallbackMesh.position.y = this.height / 2;
  ...
  async setGender(gender) {
    if (this.playerGender !== gender) {
      console.log(
        `RemotePlayer: Gender changed from ${this.playerGender} to ${gender}, reloading model...`
      );
  ...
  async _loadCharacterModel() {
    ...
    try {
      console.log(
        `RemotePlayer: Loading model for ${this.playerName} (${genderToLoad})...`
      );
      const model = await loader.loadAsync(
        basePath + `model/${genderToLoad}.fbx`
      );
        ...
        if (child.isMesh) {
          ...
          if (
            ...
            const createStandardMat = (oldMat) => {
              if (!oldMat)
                return new THREE.MeshStandardMaterial({ color: 0xff00ff }); // Magenta error
      ...
      const animPromises = ["idle", "walk", "run"].map(async (animName) => {
        try {
          const anim = await loader.loadAsync(
            basePath + `animations/${animName}.fbx`
          );
          ...
          console.warn(`RemotePlayer: Failed to load ${animName}`);
        }
      });
  ...
  update(deltaTime) {
    ...
    this.currentPosition.lerp(
      this.targetPosition,
      Math.min(1, this.lerpSpeed * deltaTime)
    );
