...
export class MenuManager {
  ...
  _initSplashScreen() {
    ...
    if (!splash) {
      // No splash screen, show menu directly
      this.elements.menuOverlay?.classList.remove('hidden');
      return;
    }
    
    // Track when splash started and when assets finished loading
    const splashStartTime = Date.now();
    const minSplashDuration = 2000; // Minimum 2 seconds (was 4)
    let assetsLoaded = false;
    let loadComplete = false;
    
    // Function to transition to headphone screen, then menu after minimum time
    const transitionToMenu = () => {
      ...
      setTimeout(() => {
        ...
        // After fade out, show headphone screen
        setTimeout(() => {
          splash.style.display = 'none';
          this._showHeadphoneScreen();
        }, 800);
      }, remainingTime + 500); // +500ms to show "Ready!" message
    };
    ...
    assetLoader.preloadAll(
      ...
      // Complete callback
      () => {
        console.log('Splash: All assets preloaded');
        assetsLoaded = true;
        if (loadingText) {
      ...
      // Error callback - continue anyway
      (error) => {
        console.warn('Splash: Some assets failed to load:', error);
        assetsLoaded = true;
        if (loadingText) {
  ...
  _showHeadphoneScreen() {
    const headphoneScreen = document.getElementById('headphone-screen');
    const continueBtn = document.getElementById('btn-headphone-continue');
    
    if (!headphoneScreen) {
      // No headphone screen, go directly to menu
      this.elements.menuOverlay?.classList.remove('hidden');
      return;
    }
    
    // Show headphone screen
    headphoneScreen.classList.remove('hidden');
    ...
    if (continueBtn) {
      ...
      continueBtn.textContent = 'WAIT...';
      
      // Enable button after minimum time
      setTimeout(() => {
        canContinue = true;
    ...
    const handleContinue = () => {
      ...
      // Unlock audio on continue
      getAudioManager().unlockAudio();
      
      headphoneScreen.style.opacity = '0';
      headphoneScreen.style.transition = 'opacity 0.5s ease-out';
      
      setTimeout(() => {
        headphoneScreen.classList.add('hidden');
        headphoneScreen.style.opacity = '';
        headphoneScreen.style.transition = '';
        this.elements.menuOverlay?.classList.remove('hidden');
      }, 500);
  ...
  startGameWithCharacter() {
    ...
    if (!this.playerProfile.playerName) {
      this.playerProfile.playerName = 'Player' + Math.floor(Math.random() * 9999);
      if (this.elements.playerNameInput) {
    ...
    console.log('Starting game as:', this.playerProfile.playerName, '-', this.playerProfile.characterGender);
  ...
  toggleVibration() {
    ...
    if (toggle) {
      ...
      const label = toggle.querySelector('.toggle-label');
      if (label) label.textContent = this.settings.vibrationEnabled ? 'ON' : 'OFF';
  ...
  applySettings() {
    // Apply quality
    this.elements.qualityButtons?.forEach(btn => {
      btn.classList.toggle('active', btn.dataset.quality === this.settings.graphicsQuality);
    });
    ...
    // Apply mic
    if (this.elements.micToggle) {
      this.elements.micToggle.classList.toggle('active', this.settings.micEnabled);
      const micLabel = this.elements.micToggle.querySelector('.toggle-label');
      if (micLabel) micLabel.textContent = this.settings.micEnabled ? 'ON' : 'OFF';
    }
    
    // Apply vibration
    if (this.elements.vibrationToggle) {
      this.elements.vibrationToggle.classList.toggle('active', this.settings.vibrationEnabled);
      const vibLabel = this.elements.vibrationToggle.querySelector('.toggle-label');
      if (vibLabel) vibLabel.textContent = this.settings.vibrationEnabled ? 'ON' : 'OFF';
    }
  ...
  applyPlayerProfile() {
    ...
    this.elements.genderButtons.forEach(btn => {
      btn.classList.toggle('active', btn.dataset.gender === this.playerProfile.characterGender);
    });
  }
  ...
  savePlayerProfile() {
    try {
      localStorage.setItem('shadowMazePlayer', JSON.stringify(this.playerProfile));
    } catch (e) {
